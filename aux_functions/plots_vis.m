function plots_vis(Global,Population,test_function, Mat_Obj)

%Retrieve data
%[~,~, ~, PF_obs, nonPF_obs, ~, ~, ~, ~, type1_obs_obj, type1_size, type2_obs_obj, type2_size,~,~,~,~,~,~ ] = sets_vis(Global, ...
%    Population, Mat_Obj);

[PF_obs, nonPF_obs, type1_obs_obj, type1_size, type2_obs_obj, type2_size] = sets_vis(Global, Population, Mat_Obj);
...
if Global.M == 2
        ...
        elseif (strcmp(test_function,'WFG1')) || (strcmp(test_function,'WFG3')) || (strcmp(test_function,'WFG4'))
         ...
         hold on
         plot(nonPF_obs(:,1),nonPF_obs(:,2),'o','MarkerSize',12,'MarkerEdgeColor','k','LineWidth',1,'MarkerFaceColor','w');
         if type1_size ~= 0
            plot(type1_obs_obj(:,1),type1_obs_obj(:,2),'^','MarkerSize',12,'MarkerEdgeColor','b','LineWidth',1,'MarkerFaceColor','b');
         end
         if type2_size ~= 0
            plot(type2_obs_obj(:,1),type2_obs_obj(:,2),'s','MarkerSize',12,'MarkerEdgeColor','r','LineWidth',1,'MarkerFaceColor','r');
         end
         hTitle = title('PF vs. ET1 and ET2 points');
         set(hTitle,'FontSize',20)
         set(gca,'FontSize',10);
         axis(size_axis);
         legend('PF','Dominated points', 'ET1', 'ET2');
