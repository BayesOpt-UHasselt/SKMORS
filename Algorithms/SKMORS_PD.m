function SKMORS_PD(Global)
    ...
    while iter > 0
        ...
        %Compute confidence bounds of predictions (hat(y))
         ...
         %% Rank points according to EOC
         %EOC = d_norm+hv_diff_norm;
         %EOC = hv_diff_norm;
         EOC = d_norm;
         
         [~,EOC_index] = sortrows(EOC,'descend');
         %dispEOC = [EOC_index];
         %dispEOC = [EOC_sorted,hv_diff_norm,d_norm];
         %disp('EOC_sorted'); disp(dispEOC); pause;

         %Best points
         top_points = EOC_index(1:5);
         %disp('top points'); disp(top_points); pause;
         
         
         %Retrieve points
         best_points = good_points(top_points); 
